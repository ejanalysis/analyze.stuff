...
#' @param na.rm Logical value, optional, TRUE by default. Defines whether NA values should be removed before min is found. Otherwise min will be NA when any NA is in a col.
...
#' rbind(blah, min=colMins2(blah), max=colMaxs(blah))
...
#' rbind(blah, min=colMins2(blah), max=colMaxs(blah), mean=colMeans(blah, na.rm=TRUE), sum=colSums(blah, na.rm=TRUE))
#' @export
