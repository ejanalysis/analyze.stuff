#' @title Show the values at 100 percentiles
#'
#' @description
#' Get a quick look at a weighted distribution by seeing the 100 values that are the percentiles 1-100
#' @details 
#' Provides weighted percentiles without using \code{\link{wtd.quantile}}
#' \cr\cr
...
#' @export
wtd.pctiles.fast.1 <- function(x, wts=NULL, na.rm=TRUE) {
  
  wts.as.pcts <- wts / sum(wts, na.rm=TRUE)
  i <- order(x) # i is a vector saying which of the x is lowest, which is next, etc. in rank small to large
  return( cumsum(wts.as.pcts[i])[order(i)] )
}
