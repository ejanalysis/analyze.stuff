...
#' @export
na.check.m = function(df, min.text=FALSE) {
  if (is.vector(df)) {stop('cannot yet handle a single vector, only a data.frame or matrix')} # would need to adjust code for this case
  if (is.matrix(df)) {df <- as.data.frame(df)} # this probably slows it down ***
  if (!is.data.frame(df)) {stop('cannot yet handle anything but data.frame or matrix')}
  # NEED TO RECODE TO HANDLE VECTOR NOT JUST DATA.FRAME, SO WOULD USE sum() not colSums() etc.
  
  cols=names(df)    # df <- as.matrix(df) # can't do that since cols can be different types
  ...
  numeric.col <- sapply(df, mode)=='numeric' # leaves out logical, complex?, character, etc.
  ...
  signtabs[ , numeric.col] <- apply(df[ , numeric.col], 2, function(x) matrixStats::signTabulate(x)[1:4] )
  #neg=  signtabs[1,]
  #zero= signtabs[2,]
  #na=   signtabs[4,]  
  ...
  neg <- signtabs[1,]   # sapply(df, function(x) signTabulate(x)[1] )
  
  inf <- sapply(df, function(x) sum(is.infinite(x), na.rm=TRUE) )
  #inf <- sum(sapply(df, is.infinite), na.rm=TRUE)

  is.char.col <- sapply(df, mode)=='character' 

  blank <- rep(0, length(df)) # i.e. once per column, only works if data frame, not matrix or vector!

  if (sum(is.char.col, na.rm=TRUE) > 1)  {blank[is.char.col] <- colSums(df[ , is.char.col]=='', na.rm=TRUE)}

  if (sum(is.char.col, na.rm=TRUE) == 1) {blank[is.char.col] <- sum(    df[ , is.char.col]=='', na.rm=TRUE)}

  not.blank.not.na <- not.na - blank

  pct.nbna <- round(100 * not.blank.not.na / count, 1)
  
  unique.not.na <- sapply(df, FUN=function(x) ifelse(any(is.na(x)), length(unique(x)) - 1, length(unique(x))))
  ...
  #min.nonzero <- ifelse(all(df==0) || all(is.na(df)), NA, colMins(df[df!=0], na.rm=TRUE)) # is slower 
  min.nonzero <- sapply(df, function(x) ifelse( (!min.text & mode(x)!='numeric') || all(x==0) || all(is.na(x)), NA, min(x[!is.na(x) & x!=0]))) #, # ~best but would be faster and return #s if avoid character cols    

  #max <- colMaxs(df,na.rm=TRUE)  # can't handle character fields
