...
#' @details 
#' Provides weighted percentiles using \code{\link{wtd.quantile}}
#' \cr\cr
...
#' @param wts NULL by default, or vector of numbers to use as weights in wtd.quantile
#' @param type 'i/n' is default. See help for \code{\link{wtd.quantile}}()
...
wtd.pctiles.exact <- function(x, wts=NULL, na.rm=TRUE, type="i/n", probs=(1:100)/100) {
  ...
  cbind(wtd.quantile(x, wts, type=type, probs=probs, na.rm=na.rm))
  ...
  # one way is not really vectorized, but just packaged as sapply(x, FUN=wtd.quantile(x, wts, type="i/n", probs=(1:100)/100, na.rm=na.rm) ) )  
