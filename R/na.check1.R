...
#' @template nachecks
#' @export
na.check1 = function(df, min.text=FALSE) {
  ...
  minNOTZERO <- sapply(df, function(x) ifelse( (!min.text & mode(x)!='numeric') || all(x==0) || all(is.na(x)), NA, min(x[!is.na(x) & x!=0])))
  # ~best but would be faster and return #s if avoid character cols

  max <- colMaxs(df,na.rm=TRUE)  # now can handle character fields
